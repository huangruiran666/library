...
public:
  static string getText(const string &key) {
    static map<string, pair<string, string>> texts = {
        ...
        {"welcome",
         {"欢迎使用专业图书管理系统 v2.0",
          "Welcome to Professional Library Management System v2.0"}},
        {"goodbye",
         {"感谢使用专业图书管理系统，再见！",
        ...
        {"sample_complete",
         {"示例数据初始化完成！", "Sample data initialization complete!"}},

        // Menu items
        {"menu_title",
         {"专业图书管理系统 v2.0",
          "Professional Library Management System v2.0"}},
        ...
        {"invalid_input",
         {"无效输入，请输入数字！", "Invalid input, please enter a number!"}},
        {"invalid_choice",
        ...
        {"book_borrowed",
         {"已被借出，无法删除。", "is borrowed and cannot be deleted."}},
        ...
        {"cannot_renew",
         {"图书已逾期，无法续借！请先归还。",
          "Book is overdue and cannot be renewed! Please return first."}},
        {"invalid_rating",
         {"评分必须在0-5之间！", "Rating must be between 0-5!"}},
...
struct Book {
  ...
  Book()
      : publicationYear(0), totalCopies(1), availableCopies(1), rating(0.0f),
        ratingCount(0), addedDate(time(nullptr)), next(nullptr) {}
};
...
struct BorrowRecord {
  ...
  BorrowRecord()
      : borrowDate(time(nullptr)), dueDate(0), returnDate(0), isReturned(false),
        fine(0.0f), next(nullptr) {}
};
...
struct User {
  ...
  User()
      : borrowLimit(5), currentBorrowed(0), isActive(true),
...
public:
  LibrarySystem()
      : bookHead(nullptr), userHead(nullptr), recordHead(nullptr),
  ...
  void saveData() {
    ...
    if (bookFile.is_open()) {
      ...
      while (current != nullptr) {
        bookFile << current->isbn << "|" << current->title << "|"
                 << current->author << "|" << current->category << "|"
                 << current->publicationYear << "|" << current->publisher << "|"
                 << current->totalCopies << "|" << current->availableCopies
                 << "|" << current->shelf << "|" << current->rating << "|"
                 << current->ratingCount << "|" << current->description << "|"
    ...
    if (userFile.is_open()) {
      ...
      while (current != nullptr) {
        userFile << current->userId << "|" << current->name << "|"
                 << current->email << "|" << current->phone << "|"
                 << current->userType << "|" << current->borrowLimit << "|"
                 << current->currentBorrowed << "|" << current->isActive << "|"
    ...
    if (recordFile.is_open()) {
      ...
      while (current != nullptr) {
        recordFile << current->isbn << "|" << current->userId << "|"
                   << current->bookTitle << "|" << current->userName << "|"
                   << current->borrowDate << "|" << current->dueDate << "|"
                   << current->returnDate << "|" << current->isReturned << "|"
                   << current->fine << endl;
        current = current->next;
      }
      recordFile.close();
  ...
  void addBook(const string &isbn, const string &title, const string &author,
               ...
               int copies, const string &shelf,
               const string &description = "") {
    if (findBookByISBN(isbn) != nullptr) {
      cout << "❌ " << LanguageText::getText("error") << ": "
           << LanguageText::getText("isbn_exists") << endl;
      return;
    }
    ...
    if (currentLanguage == ENGLISH)
      msg = LanguageText::getText("book_added") + ": \"" + title + "\"";
    cout << "✓ " << msg << " (" << LanguageText::getText("total") << " "
         << copies << " " << LanguageText::getText("books") << ")" << endl;
  }
  ...
  void rateBook(const string &isbn, float rating) {
    ...
    cout << "✓ 评分成功！《" << book->title << "》 当前评分：" << fixed
         << setprecision(1) << book->rating << " (" << book->ratingCount
         << "人评价)" << endl;
  }
  ...
  void displayAllUsers() {
    ...
    cout << left << setw(15) << "用户ID" << setw(20) << "姓名" << setw(30)
         << "邮箱" << setw(15) << "电话" << setw(12) << "类型" << setw(10)
         << "已借/限额"
         << "状态" << endl;
    cout << string(120, '-') << endl;
    ...
    User *current = userHead;
    while (current != nullptr) {
      cout << left << setw(15) << current->userId << setw(20) << current->name
           << setw(30) << current->email << setw(15) << current->phone
           << setw(12) << current->userType << setw(10)
           << (to_string(current->currentBorrowed) + "/" +
               to_string(current->borrowLimit))
  ...
  void borrowBook(const string &isbn, const string &userId) {
    ...
    if (user->currentBorrowed >= user->borrowLimit) {
      cout << "❌ 错误：已达到借阅上限（" << user->borrowLimit << "本）！"
           << endl;
      return;
    }
  ...
  void returnBook(const string &isbn, const string &userId) {
    ...
    BorrowRecord *current = recordHead;
    while (current != nullptr) {
      if (current->isbn == isbn && current->userId == userId &&
          !current->isReturned) {
        ...
        if (current->returnDate > current->dueDate) {
          int overdueDays =
              getDaysDifference(current->dueDate, current->returnDate);
        ...
        if (current->fine > 0) {
          int overdueDays =
              getDaysDifference(current->dueDate, current->returnDate);
          cout << "⚠️  逾期 " << overdueDays << " 天，罚款：￥" << fixed
               << setprecision(2) << current->fine << endl;
        } else {
          cout << "✓ 按时归还，无罚款" << endl;
        }
        return;
  ...
  void renewBook(const string &isbn, const string &userId) {
    ...
    while (current != nullptr) {
      if (current->isbn == isbn && current->userId == userId &&
        ...
        cout << "✓ 续借成功！新的应还日期：" << getTimeString(current->dueDate)
             << endl;
        return;
      }
      current = current->next;
  ...
  void advancedSearch() {
    ...
    while (current != nullptr) {
      ...
      if (!title.empty() &&
          toLower(current->title).find(toLower(title)) == string::npos)
        match = false;
      if (!author.empty() &&
          toLower(current->author).find(toLower(author)) == string::npos)
        match = false;
      if (!category.empty() &&
          toLower(current->category).find(toLower(category)) == string::npos)
        match = false;
      if (!publisher.empty() &&
          toLower(current->publisher).find(toLower(publisher)) == string::npos)
        match = false;
      if (current->rating < minRating)
        match = false;
      if (current->publicationYear < yearFrom ||
          current->publicationYear > yearTo)
        match = false;
  ...
  void displayBookDetails(Book *book) {
    ...
    cout << "   藏书量: " << book->totalCopies
         << " 本 (可借: " << book->availableCopies << " 本)" << endl;
  ...
  void displayBorrowedBooks() {
    ...
    cout << "\n📕 当前借阅中的图书：" << endl;
    cout << string(120, '=') << endl;
    cout << left << setw(20) << "图书标题" << setw(15) << "借阅人" << setw(12)
         << "用户ID" << setw(22) << "借阅日期" << setw(22) << "应还日期"
         << "状态" << endl;
    cout << string(120, '-') << endl;
    ...
    while (current != nullptr) {
      if (!current->isReturned) {
        cout << left << setw(20) << current->bookTitle.substr(0, 18) << setw(15)
             << current->userName.substr(0, 13) << setw(12) << current->userId
  ...
  void displayUserBorrowHistory(const string &userId) {
    ...
    cout << "\n📚 借阅历史：" << endl;
    cout << string(120, '=') << endl;
    cout << left << setw(22) << "图书标题" << setw(22) << "借阅日期" << setw(22)
         << "应还日期" << setw(22) << "实际归还日期" << setw(10) << "罚款"
         << "状态" << endl;
    cout << string(120, '-') << endl;

    for (BorrowRecord *rec : userRecords) {
      cout << left << setw(22) << rec->bookTitle.substr(0, 20) << setw(22)
           << getTimeString(rec->borrowDate).substr(0, 20) << setw(22)
           << getTimeString(rec->dueDate).substr(0, 20) << setw(22)
           << getTimeString(rec->returnDate).substr(0, 20);
  ...
  void displayOverdueBooks() {
    ...
    if (overdueRecords.empty()) {
      ...
      cout << left << setw(22) << "图书标题" << setw(15) << "借阅人" << setw(12)
           << "用户ID" << setw(22) << "应还日期" << setw(12) << "逾期天数"
           << "预计罚款" << endl;
      cout << string(120, '-') << endl;
      ...
      for (BorrowRecord *rec : overdueRecords) {
        ...
        cout << left << setw(22) << rec->bookTitle.substr(0, 20) << setw(15)
             << rec->userName.substr(0, 13) << setw(12) << rec->userId
             << setw(22) << getTimeString(rec->dueDate).substr(0, 20)
             << setw(12) << (to_string(overdueDays) + "天") << "￥" << fixed
             << setprecision(2) << estimatedFine << endl;
      }
      cout << string(120, '=') << endl;
  ...
  void displayPopularBooks() {
    ...
    vector<pair<string, int>> sortedBooks(borrowCount.begin(),
                                          borrowCount.end());
    ...
    cout << "\n🔥 热门图书排行榜（按借阅次数）：" << endl;
    cout << string(100, '=') << endl;
    cout << left << setw(8) << "排名" << setw(35) << "书名" << setw(20)
         << "作者" << setw(12) << "借阅次数"
         << "评分" << endl;
    cout << string(100, '-') << endl;
    ...
    for (const auto &pair : sortedBooks) {
      ...
      if (book != nullptr) {
        cout << left << setw(8) << rank << setw(35) << book->title.substr(0, 33)
             << setw(20) << book->author.substr(0, 18) << setw(12)
             << pair.second;
  ...
  void displayCategoryStatistics() {
    ...
    while (current != nullptr) {
      ...
      categoryBorrowed[current->category] +=
          (current->totalCopies - current->availableCopies);
      current = current->next;
    }
    ...
    cout << "\n📊 分类统计：" << endl;
    cout << string(80, '=') << endl;
    cout << left << setw(20) << "分类" << setw(15) << "图书总数" << setw(15)
         << "已借出" << setw(15) << "可借阅"
         << "借出率" << endl;
    cout << string(80, '-') << endl;
    ...
    for (const auto &pair : categoryCount) {
      ...
      cout << left << setw(20) << pair.first << setw(15) << total << setw(15)
           << borrowed << setw(15) << available << fixed << setprecision(1)
           << borrowRate << "%" << endl;
    }
    cout << string(80, '=') << endl;
  }
  ...
  void exportToCSV() {
    ...
    // CSV Header
    file << "ISBN,标题,作者,分类,出版年份,出版社,总数,可借,书架,评分,评价数"
         << endl;

    Book *current = bookHead;
    while (current != nullptr) {
      file << current->isbn << "," << current->title << "," << current->author
           << "," << current->category << "," << current->publicationYear << ","
           << current->publisher << "," << current->totalCopies << ","
           << current->availableCopies << "," << current->shelf << "," << fixed
           << setprecision(2) << current->rating << "," << current->ratingCount
           << endl;
      current = current->next;
    }
    ...
    while (getline(file, line)) {
      ...
      if (parts.size() >= 9) {
        if (findBookByISBN(parts[0]) == nullptr) {
          addBook(parts[0], parts[1], parts[2], parts[3], stoi(parts[4]),
                  parts[5], stoi(parts[6]), parts[8]);
          importCount++;
        }
      }
    }
...
void displayMenu() {
  ...
  cout << line << "           📚 " << LanguageText::getText("menu_title")
       << " 📚                      " << line << endl;
  cout << split[0] << string(60, border[0]) << split[1] << endl;
  cout << line << "  📖 " << LanguageText::getText("menu_book_mgmt")
       << string(currentLanguage == CHINESE ? 47 : 34, ' ') << line << endl;
  ...
  cout << line << "    4. " << LanguageText::getText("menu_view_all")
       ...
       << string(currentLanguage == CHINESE ? 4 : 1, ' ') << "6. "
       << LanguageText::getText("menu_rate")
       << string(currentLanguage == CHINESE ? 6 : 8, ' ') << line << endl;
  cout << split[0] << string(60, border[0]) << split[1] << endl;
  cout << line << "  👥 " << LanguageText::getText("menu_user_mgmt")
       << string(currentLanguage == CHINESE ? 47 : 35, ' ') << line << endl;
  cout << line << "    7. " << LanguageText::getText("menu_add_user")
       << string(10, ' ') << "8. " << LanguageText::getText("menu_view_user")
       << string(currentLanguage == CHINESE ? 2 : 0, ' ') << "9. "
       << LanguageText::getText("menu_all_users")
       << string(currentLanguage == CHINESE ? 2 : 0, ' ') << line << endl;
  cout << split[0] << string(60, border[0]) << split[1] << endl;
  cout << line << "  📋 " << LanguageText::getText("menu_borrow_mgmt")
       << string(currentLanguage == CHINESE ? 43 : 27, ' ') << line << endl;
  cout << line << "   10. " << LanguageText::getText("menu_borrow")
       << string(9, ' ') << "11. " << LanguageText::getText("menu_return")
       << string(currentLanguage == CHINESE ? 5 : 2, ' ') << "12. "
       << LanguageText::getText("menu_renew")
  ...
  cout << line << "   13. " << LanguageText::getText("menu_current")
       ...
       << string(currentLanguage == CHINESE ? 5 : 2, ' ') << "15. "
       << LanguageText::getText("menu_history")
       << string(currentLanguage == CHINESE ? 5 : 0, ' ') << line << endl;
  cout << split[0] << string(60, border[0]) << split[1] << endl;
  cout << line << "  🔍 " << LanguageText::getText("menu_search")
       << string(currentLanguage == CHINESE ? 47 : 32, ' ') << line << endl;
  cout << line << "   16. " << LanguageText::getText("menu_search_title")
       << string(currentLanguage == CHINESE ? 7 : 0, ' ') << "17. "
       << LanguageText::getText("menu_search_author")
       << string(currentLanguage == CHINESE ? 3 : 0, ' ') << "18. "
       << LanguageText::getText("menu_search_category")
       << string(currentLanguage == CHINESE ? 3 : 0, ' ') << line << endl;
  cout << line << "   19. " << LanguageText::getText("menu_advanced_search")
       << string(currentLanguage == CHINESE ? 51 : 33, ' ') << line << endl;
  cout << split[0] << string(60, border[0]) << split[1] << endl;
  cout << line << "  📊 " << LanguageText::getText("menu_statistics")
       << string(currentLanguage == CHINESE ? 43 : 25, ' ') << line << endl;
  cout << line << "   20. " << LanguageText::getText("menu_system_stats")
       << string(currentLanguage == CHINESE ? 8 : 1, ' ') << "21. "
       << LanguageText::getText("menu_popular")
       << string(currentLanguage == CHINESE ? 5 : 2, ' ') << "22. "
       << LanguageText::getText("menu_category_stats")
       << string(currentLanguage == CHINESE ? 5 : 0, ' ') << line << endl;
  cout << split[0] << string(60, border[0]) << split[1] << endl;
  cout << line << "  💾 " << LanguageText::getText("menu_data_mgmt")
       << string(currentLanguage == CHINESE ? 47 : 35, ' ') << line << endl;
  cout << line << "   23. " << LanguageText::getText("menu_save")
       << string(currentLanguage == CHINESE ? 9 : 3, ' ') << "24. "
       << LanguageText::getText("menu_export")
       << string(currentLanguage == CHINESE ? 6 : 2, ' ') << "25. "
       << LanguageText::getText("menu_import")
       << string(currentLanguage == CHINESE ? 6 : 3, ' ') << line << endl;
  cout << split[0] << string(60, border[0]) << split[1] << endl;
  cout << line << "   0. " << LanguageText::getText("menu_exit")
       << string(currentLanguage == CHINESE ? 52 : 38, ' ') << line << endl;
...
int main() {
  ...
  if (library.findBookByISBN("978-0-13-110362-7") == nullptr) {
    ...
    if (currentLanguage == CHINESE) {
      library.addBook("978-0-13-110362-7", "C程序设计语言",
                      "Kernighan & Ritchie", "编程", 1988, "机械工业出版社", 3,
                      "A1", "C语言的权威指南，由语言设计者编写。");
      library.addBook("978-0-13-235088-4", "代码整洁之道", "Robert C. Martin",
                      "编程", 2008, "人民邮电出版社", 5, "A2",
                      "软件工程经典之作，讲述如何编写高质量代码。");
      library.addBook("978-0-20163-361-0", "设计模式", "Gamma等", "编程", 1994,
                      "机械工业出版社", 2, "A3",
                      "面向对象设计的经典著作，GoF四人帮力作。");
      library.addBook("978-7-11-545800-5", "三体", "刘慈欣", "科幻小说", 2008,
                      "重庆出版社", 10, "B1", "中国科幻巨著，雨果奖获奖作品。");
      library.addBook("978-0-45-152493-5", "指环王", "J.R.R. Tolkien",
                      "奇幻小说", 1954, "译林出版社", 8, "B2",
                      "史诗级奇幻小说，现代奇幻文学的奠基之作。");
      library.addBook("978-0-13-468599-1", "深入理解计算机系统",
                      "Randal E. Bryant", "计算机科学", 2015, "机械工业出版社",
                      4, "A4", "计算机系统经典教材，深入讲解计算机底层原理。");
      library.addBook("978-7-115-42848-3", "Python编程：从入门到实践",
                      "Eric Matthes", "编程", 2016, "人民邮电出版社", 6, "A5",
                      "Python入门经典教材，适合初学者。");
      ...
      library.addBook(
          ...
          "Kernighan & Ritchie", "Programming", 1988, "Prentice Hall", 3, "A1",
      ...
      library.addBook(
          "978-7-11-545800-5", "The Three-Body Problem", "Liu Cixin",
          "Science Fiction", 2008, "Tor Books", 10, "B1",
          "Hugo Award-winning Chinese science fiction masterpiece.");
      library.addBook(
          "978-0-45-152493-5", "The Lord of the Rings", "J.R.R. Tolkien",
          "Fantasy", 1954, "Allen & Unwin", 8, "B2",
          "Epic high-fantasy novel, foundation of modern fantasy literature.");
      library.addBook("978-0-13-468599-1",
                      "Computer Systems: A Programmer's Perspective",
                      "Randal E. Bryant", "Computer Science", 2015, "Pearson",
                      4, "A4", "Classic textbook on computer systems.");
      library.addBook("978-7-115-42848-3", "Python Crash Course",
                      "Eric Matthes", "Programming", 2016, "No Starch Press", 6,
                      "A5",
                      "A hands-on, project-based introduction to programming.");
      library.addBook("978-0-596-52068-7", "Introduction to Algorithms",
                      "Thomas H. Cormen", "Computer Science", 2009, "MIT Press",
                      3, "A6", "Comprehensive introduction to algorithms.");
    }

    // Add sample users
    if (currentLanguage == CHINESE) {
      library.addUser("U001", "张三", "zhangsan@example.com", "13800138000",
                      "student");
      library.addUser("U002", "李四", "lisi@example.com", "13800138001",
                      "teacher");
      library.addUser("U003", "王五", "wangwu@example.com", "13800138002",
                      "student");
      library.addUser("ADMIN", "管理员", "admin@library.com", "13900000000",
                      "admin");
    } else {
      library.addUser("U001", "John Smith", "john@example.com",
                      "1-800-111-0001", "student");
      library.addUser("U002", "Mary Johnson", "mary@example.com",
                      "1-800-111-0002", "teacher");
      library.addUser("U003", "Bob Williams", "bob@example.com",
                      "1-800-111-0003", "student");
      library.addUser("ADMIN", "Administrator", "admin@library.com",
                      "1-800-111-9999", "admin");
    }
  ...
  while (true) {
    ...
    // ==================== BOOK MANAGEMENT ====================
    case 1: { // Add book
      cout << "\n╔══════════ " << LanguageText::getText("menu_add_book")
           << " ══════════╗" << endl;
      ...
      library.addBook(isbn, title, author, category, year, publisher, copies,
                      shelf, description);
      break;
    }
    case 2: { // Remove book
      cout << "\n╔══════════ " << LanguageText::getText("menu_del_book")
           << " ══════════╗" << endl;
      cout << LanguageText::getText("isbn") << ": ";
      getline(cin, isbn);
    ...
    case 3: { // Update book
      cout << "\n╔══════════ " << LanguageText::getText("menu_update_book")
           << " ══════════╗" << endl;
      cout << LanguageText::getText("isbn") << ": ";
      getline(cin, isbn);
    ...
    // ==================== USER MANAGEMENT ====================
    case 7: { // Add user
      cout << "\n╔══════════ " << LanguageText::getText("menu_add_user")
           << " ══════════╗" << endl;
    ...
    case 8: { // Display user info
      cout << "\n╔══════════ " << LanguageText::getText("menu_view_user")
           << " ══════════╗" << endl;
      cout << LanguageText::getText("user_id") << ": ";
      getline(cin, userId);
    ...
    // ==================== BORROW MANAGEMENT ====================
    case 10: { // Borrow book
      cout << "\n╔══════════ " << LanguageText::getText("menu_borrow")
           << " ══════════╗" << endl;
      cout << LanguageText::getText("isbn") << ": ";
      getline(cin, isbn);
      cout << LanguageText::getText("user_id") << ": ";
    ...
    case 11: { // Return book
      cout << "\n╔══════════ " << LanguageText::getText("menu_return")
           << " ══════════╗" << endl;
      cout << LanguageText::getText("isbn") << ": ";
      getline(cin, isbn);
      cout << LanguageText::getText("user_id") << ": ";
    ...
    case 16: { // Search by title
      cout << "\n╔══════════ " << LanguageText::getText("menu_search_title")
           << " ══════════╗" << endl;
      cout << LanguageText::getText("title") << ": ";
      getline(cin, title);
